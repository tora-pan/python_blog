...
from pythonblog.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm
...
def save_picture(picture_data):
    ...
    output_size = (125,125)
    new_image = Image.open(picture_data)
...
@login_required
def update_post(post_id, methods=["GET"]):
    post = Post.query.get_or_404(post_id)
...
@login_required
def delete_post(post_id, methods=["POST"]):
    post = Post.query.get_or_404(post_id)
    if post.author != current_user:
        abort(403)
